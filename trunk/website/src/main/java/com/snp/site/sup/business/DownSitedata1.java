...
public class DownSitedata1 implements IbusinessAction {
	private static Log log = LogFactory.getLog(DownSitedata1.class);
	...
	public void excute(IOperationContext context) throws OperationException {
		try {
			...
			String exportpath = WebAppContextUtils.getpara("exportpath");
			// String importpath = WebAppContextUtils.getpara("importpath");
			...
			for (Iterator iterator = siteUser.getSitePresources().iterator(); iterator
					.hasNext();) {
				SitePresource sitePresource = (SitePresource) iterator.next();
				try {
					FileUtils.copyFile(
							new File(SystemInit.getWebroot() + "resself/"
									+ sitePresource.getFilepath()),
							new File(userhome + "/resself/"
									+ sitePresource.getFilepath()));
				} catch (Exception e) {
				}
			}
			...
			String objectFilePath = userhome + "/" + siteUser.getUsername()
					+ ".dat";
			...
			Compress.zip(userhome, exportpath + siteUser.getUsername() + ".zip");
