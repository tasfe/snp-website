...
public class DownSitedata implements IbusinessAction {
	...
	public void excute(IOperationContext context) throws OperationException {
		try {
			...
			SiteUser siteUser = (SiteUser) adao.loadById("SiteUser",
					Long.valueOf(WebAppContextUtils.getpara("userid")));
			listSiteUser(siteUser);// 因为懒加载的原因，LIST一下进行加载
			String srcDir = SystemInit.getSiteHome() + siteUser.getUsername();
			String destDir = SystemInit.getWebroot() + "bak/"
					+ siteUser.getUsername();
			FileUtils.deleteDirectory(new File(destDir));
			FileUtils.forceMkdir(new File(SystemInit.getWebroot() + "bak"));
			FileUtils.forceMkdir(new File(destDir));
			FileUtils.forceMkdir(new File(srcDir));
			String objectFilePath = srcDir + "/" + siteUser.getUsername()
					+ ".dat";

			ObjectOutputStream out = new ObjectOutputStream(
					new FileOutputStream(objectFilePath));
			out.writeObject(siteUser);
			out.close();
			/*
			 * for (Iterator iterator = siteUser.getSitePresources().iterator();
			 * iterator.hasNext();) { SitePresource sitePresource =
			 * (SitePresource) iterator.next(); try { FileUtils.copyFile(new
			 * File(SystemInit.getWebroot() +
			 * "resself/"+sitePresource.getFilepath()),new File(destDir+
			 * "/resself/"+sitePresource.getFilepath())); } catch (Exception e)
			 * { } }
			 */

			FileUtils.copyDirectory(new File(srcDir), new File(destDir
					+ "/site/" + siteUser.getUsername()));

			FileUtils.copyDirectory(
					new File(SystemInit.getWebroot() + "other/"), new File(
							destDir + "/other/"));
			FileProcessor
					.strToDoc(
							destDir + "/" + "/index.html",
							"<script type=\"text/javascript\" language=\"javascript\"> document.location=\"site/test/index_1.html\";</script>");
			Compress.zip(destDir,
					SystemInit.getWebroot() + "bak/" + siteUser.getUsername()
							+ ".zip");
