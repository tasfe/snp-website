...
<div class="content">
	<form id="add_form" action='' method="post">   		
	   ...
	   &nbsp;&nbsp;<a  class="bt"  href="#" onclick="javascript:$('#add_form').submit();" >插入文件类别</a>	
	</form> 		    
	<br>
        ...
        <tr>
            <td class=td-data valign=top>
		 	...
		 	<td class=td-op style="width:140px">
				...
				上传文件</a>						 	   
		    	<a class=bt href="javascript:modify(${results["DM.Reload.SiteUser.Result"].id},${x.id},'true');">
		                            编辑描述
			    </a>		
			    <#if x.siteSpmp5Items?exists&&(x.siteSpmp5Items?size!=0)>
			       <br>  <br> <a class=bt title='管理文件（文件名称修改/单个文件描述修改/删除替换文件）'  href="javascript:detail('${x.id}')"> 
			                       编辑文件信息
				    </a>(${x.siteSpmp5Items?size}个文件)
				</#if>
				<br>
                <#--
			   &nbsp;
				...
				-->
				<br>
                 <span title='删除该文件目录' onclick="javascript:del(${x.id});" class="snp-icon ui-icon-trash"></span> 
...
<@spmpadmin nob=5 lanmutemplete="spmp5.ftl"  adminftl="site_spmp5_admin.ftl" title=results["DM.Reload.SiteUser.Result"].spmp5Lanmu  spmp=results["DM.Reload.SiteUser.Result"].siteSpmp5s/>
